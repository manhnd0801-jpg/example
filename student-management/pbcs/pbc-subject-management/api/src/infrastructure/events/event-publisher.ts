...
export class EventPublisher implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly config: ConfigService) {
    const kafka = new Kafka({ clientId: this.config.get('KAFKA_CLIENT_ID', 'pbc-subject-management'), brokers: this.config.getOrThrow<string>('KAFKA_BROKERS').split(',') });
  ...
  async publish(topic: string, data: Record<string, unknown>, tenantId: string, correlationId: string) {
    ...
    try { await this.producer.send({ topic, messages: [{ key: tenantId, value: JSON.stringify(envelope) }] }); }
    catch (err) { this.logger.error(`Failed to publish ${topic}: ${(err as Error).message}`); }
  }
}
