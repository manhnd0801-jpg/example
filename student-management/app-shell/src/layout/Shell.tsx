...
const LazyUserManagement = lazy(() => {
  ...
  return importFromRemote<React.ComponentType>(authEntry, "./UserManagementSlot").then((C) => ({
    default: C,
  }));
});
...
export const Shell: React.FC<Props> = ({ children }) => {
  ...
  const menuItems: MenuProps["items"] = [
    {
      ...
      label: <a href="/dashboard" style={{ textDecoration: "none" }}>Dashboard</a>,
    },
    { type: "divider" },
    // PBC nghiệp vụ
    ...pbcs.map((pbc) => ({
      ...
      label: (
        <a href={pbc.routePrefix} style={{ textDecoration: "none" }}>
          {PBC_CONFIG[pbc.pbcId]?.label ?? pbc.pbcId}
        </a>
      ),
    })),
            ...
            children: [
              {
                ...
                icon: <TeamOutlined />,
                label: (
                  <a href="/user-management" style={{ textDecoration: "none" }}>
                    Quản lý người dùng
                  </a>
                ),
              },
  ...
  // Render content: /user-management → UserManagementSlot, còn lại → children
  const renderContent = () => {
